#!/usr/bin/env python3
"""Generate breed pages for petexpenses.com.

Usage:
  python3 generate_breeds.py                    # Generate all 20 pages
  python3 generate_breeds.py --breed "French Bulldog"  # Single page
"""

import os, sys, json, re, urllib.request, urllib.parse

# ─── PET DATA (from index.html) ─────────────────────────────────
...
def get_similar_breeds(breed_name, breed_data, count=5):
    size = breed_data[0]
    similar = []
    for name, data in DOG_BREEDS.items():
...
def build_args(name, data):
    ...
    food = c['food'][size]
    vet = c['vet'][size]
    ins = c['insurance'][size]
    groom = c['grooming'][coat][size]
    supp = c['supplies'][size]
    ...
    sd = {'small': 'small', 'medium': 'medium-sized', 'large': 'large', 'giant': 'giant'}[size]
    cd = {'short': 'short-coated', 'long': 'long-coated', 'wire': 'wire-haired'}[coat]
    bp = name + ('es' if name[-1] in 'sx' else 's')
    dw = {'small': 12, 'medium': 35, 'large': 65, 'giant': 110}[size]
    ...
    # Health extended
    if any(w in health_note.lower() for w in ['breathing', 'brachycephalic']):
        he = f'{bp} are brachycephalic (flat-faced), which means they are prone to breathing difficulties, heat sensitivity, and require careful monitoring during exercise and hot weather.'
    elif 'hip' in health_note.lower():
        he = f'{bp} are prone to hip and joint issues, which may require ongoing supplements, medications, or surgical intervention as they age.'
    elif 'cancer' in health_note.lower():
        he = f'{bp} have a higher-than-average cancer risk, making regular veterinary screenings and early detection particularly important.'
    elif 'dental' in health_note.lower():
        he = f'{bp} are prone to dental issues, which may require professional cleanings and at-home care.'
    elif 'heart' in health_note.lower() or 'cardiac' in health_note.lower():
        he = f'{bp} are prone to cardiac conditions, which require regular monitoring and can add significant veterinary costs over their lifetime.'
    else:
        he = f'{bp} are generally healthy but can be prone to {hi_lc}.'

    # Grooming
    if coat == 'long':
        gt = f'With their {cd} coat, {bp} require regular brushing 2-3 times per week and professional grooming every 6-8 weeks. Annual grooming costs range from ${gl} to ${gh}. Regular grooming prevents matting and skin issues.'
    elif coat == 'wire':
        gt = f'{bp} have a distinctive wire coat that needs hand-stripping or professional grooming every 8-10 weeks. Budget ${gl}–${gh}/year for professional grooming, plus at-home brushing.'
    else:
        gt = f'{bp} have a short, low-maintenance coat. Brushing once a week is sufficient. Professional grooming is rarely needed — annual costs are just ${gl}–${gh}, mainly for nail trims and ear cleaning.'

    avg = 2800
    diff = ((cl + ch) / 2) - avg
    if diff > 500:
        cc = 'More'; cc_lc = 'more'; cr = f'their {sd} size, breed-specific health issues, and specialized care needs'
    elif diff < -500:
        cc = 'Less'; cc_lc = 'less'; cr = f'their {sd} size, generally good health, and low-maintenance care requirements'
    else:
        cc = 'About Average'; cc_lc = 'about average'; cr = f'their {sd} size and moderate health profile'

    cl_label = 'more' if cc == 'More' else 'less'
    hf = f'{bp} cost {cl_label} than the average dog primarily because of their {sd} size and breed-specific health considerations. {he} Additionally, {bp.lower()} often require {cd} maintenance, which affects annual grooming costs.'

    # Cost comparison sentence for FAQ
    if cc == 'About Average':
        comp_sentence = f'{bp} cost about the same as the average dog. A typical medium-sized mixed breed costs around $1,500–$3,000 per year, and {bp.lower()} at ${cl:,}–${ch:,}/year fall right in line due to {cr}.'
    elif cc == 'More':
        comp_sentence = f'{bp} are more expensive than average to own. A typical medium-sized mixed breed costs around $1,500–$3,000 per year, while {bp.lower()} at ${cl:,}–${ch:,}/year cost more due to {cr}.'
    else:
        comp_sentence = f'{bp} are less expensive than average to own. A typical medium-sized mixed breed costs around $1,500–$3,000 per year, while {bp.lower()} at ${cl:,}–${ch:,}/year cost less due to {cr}.'

    # First year extra
    fye = int((cl + ch) / 2 * 0.4)

    # Similar breeds
    sim = get_similar_breeds(name, data, 4)
    sim_links = ''
    for sib in sim:
        ss = make_slug(sib)
        sim_links += f'<a href="/breeds/{ss}">Dogs {sib}</a>\n    '

    md = f'See the real annual cost of owning a {name} in 2026. Food, vet, insurance, grooming & supplies: ${cl:,}–${ch:,}/year. Free breed-specific calculator included.'[:160]

    # Plural for H1 article
    h1_article = art
    if name.lower() == 'australian shepherd':
        h1_article = 'an'
    ...
    return {
        ...
        'FOOD_LOW': str(fl),
        'FOOD_HIGH': str(fh),
        'FOOD_MONTHLY_LOW': str(fl // 12),
        'FOOD_MONTHLY_HIGH': str(fh // 12),
        'FOOD_PCT': str(pct(fl, fh)),
        'VET_LOW': str(vl),
        'VET_HIGH': str(vh),
        'VET_MONTHLY_LOW': str(vl // 12),
        'VET_MONTHLY_HIGH': str(vh // 12),
        'VET_PCT': str(pct(vl, vh)),
        'INS_LOW': str(il),
        'INS_HIGH': str(ih),
        'INS_MONTHLY_LOW': str(il // 12),
        'INS_MONTHLY_HIGH': str(ih // 12),
        'INS_PCT': str(pct(il, ih)),
        'GROOM_LOW': str(gl),
        'GROOM_HIGH': str(gh),
        'GROOM_MONTHLY_LOW': str(gl // 12),
        'GROOM_MONTHLY_HIGH': str(gh // 12),
        'GROOM_PCT': str(pct(gl, gh)),
        'SUPP_LOW': str(sl),
        'SUPP_HIGH': str(sh),
        'SUPP_MONTHLY_LOW': str(sl // 12),
        'SUPP_MONTHLY_HIGH': str(sh // 12),
...
# ═══════════════════════════════════════════════════════════════
# HTML TEMPLATE (uses !KEY! style placeholders)
# ═══════════════════════════════════════════════════════════════
    ...
    },
    {
      ...
      "mainEntity": [
        ...
        },
        {
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes — !BREED!s are prone to !HEALTH_ISSUES_LC!, which can lead to expensive vet bills. One emergency surgery can cost $3,000–$10,000. Pet insurance typically costs $!INS_LOW!–$!INS_HIGH!/year and can cover 70-90% of eligible costs."
          }
        },
        {
          "@type": "Question",
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Food and veterinary care are the largest expenses for !BREED!s. Food costs $!FOOD_LOW!–$!FOOD_HIGH!/year, while vet care ranges from $!VET_LOW!–$!VET_HIGH!/year including routine check-ups, vaccinations, and breed-specific health concerns."
          }
        }
      ]
    }
...
<meta property="og:image" content="https://petexpenses.com/og-dog.jpg">
...
.article-body table{width:100%;border-collapse:collapse;margin:24px 0;font-size:.95rem}
...
.article-cta{display:flex;justify-content:center;margin-top:48px;padding:32px;background:var(--paper);border:2.5px solid var(--ink);border-radius:var(--radius-xl);box-shadow:6px 6px 0 var(--coral);text-align:center}
.article-cta-text{font-family:var(--display);font-weight:800;font-size:1.3rem;margin-bottom:12px}
.offers-section{max-width:780px;margin:48px auto 0;padding:0 24px}
...
.cost-table th{background:var(--ink);color:var(--cream);padding:10px 14px;text-align:left;font-family:var(--mono);font-size:11px;letter-spacing:.08em;text-transform:uppercase}
.cost-table td{padding:10px 14px;border-bottom:1px solid rgba(27,19,64,.08)}
  ...
  <article>
    <header class="article-header">
      ...
      <div class="article-meta">
        ...
        <span>!BREED_PLURAL! guide</span>
      </div>
    </header>

    <div class="article-body">

      <div class="highlight-card hc-coral">
        <div class="hc-label">Quick Answer</div>
        <p>The average annual cost of owning a !BREED! in the US ranges from <strong>$!COST_LOW! to $!COST_HIGH! per year</strong> ($!MONTHLY_LOW!–$!MONTHLY_HIGH!/month). This includes food, routine vet care, pet insurance, grooming, and supplies. Actual costs depend on your dog&rsquo;s age, weight, diet, activity level, and your location.</p>
      </div>

      <h2>Annual Cost Breakdown for !BREED_PLURAL!</h2>
      <p>Here&rsquo;s how the average !BREED! owner&rsquo;s annual budget breaks down across five key categories. !BREED_PLURAL! are a <strong>!SIZE_DESC!</strong> breed with a <strong>!COAT_DESC!</strong> coat.</p>
      ...
      <div class="cost-table-wrap">
        <table class="cost-table">
          ...
          <tbody>
            <tr><td>Food</td><td>$!FOOD_LOW!–$!FOOD_HIGH!</td><td>$!FOOD_MONTHLY_LOW!–$!FOOD_MONTHLY_HIGH!</td><td>!FOOD_PCT!%</td></tr>
      ...
      <h3>Food</h3>
      <p>Food is typically the largest recurring expense for !BREED_PLURAL!. Expect to spend $!FOOD_LOW!–$!FOOD_HIGH! per year on quality kibble or wet food. !BREED_PLURAL! with food sensitivities or allergies may need specialized diets, which can add !FOOD_ALLERGY_EXTRA! per year. Larger !BREED_PLURAL! eat more and therefore cost more to feed.</p>

      <h3>Veterinary Care</h3>
      <p>Routine veterinary care for !BREED_PLURAL! costs $!VET_LOW!–$!VET_HIGH! per year. This covers annual check-ups, vaccinations, heartworm prevention, and flea/tick treatment. !HEALTH_NOTE_EXTENDED! Unexpected emergencies — like a foreign body removal, injury, or sudden illness — can add thousands of dollars in a single visit.</p>

      <h3>Pet Insurance</h3>
      <p>Pet insurance for a !BREED! costs $!INS_LOW!–$!INS_HIGH! per year ($!INS_MONTHLY_LOW!–$!INS_MONTHLY_HIGH!/month). Given the breed&rsquo;s predisposition to !HEALTH_ISSUES_LC!, insurance is worth considering. A single surgery or emergency visit can cost $3,000 to $10,000 — far more than a year&rsquo;s premiums.</p>

      <h3>Grooming</h3>
      <p>!GROOMING_TEXT!</p>

      <h3>Supplies</h3>
      <p>Annual supplies — leash, collar, bed, bowls, toys, poop bags, and basic accessories — run $!SUPP_LOW!–$!SUPP_HIGH!. Initial setup in the first year costs more due to one-time purchases like a crate, bed, and initial vet visit.</p>

      <h2>Why !BREED_PLURAL! Cost More Than Average</h2>
      ...
      <ul>
        ...
        <li><strong>Compare pet insurance plans.</strong> Get quotes from at least three providers. Accident-only plans start around $!INS_LOW!/year, while comprehensive plans with wellness coverage are more expensive but may save money long-term.</li>
        <li><strong>Buy food in bulk.</strong> High-quality kibble in larger bags (15-30 lb) is significantly cheaper per pound. Subscribe to auto-ship for additional savings.</li>
        <li><strong>DIY grooming where safe.</strong> Nail trims, ear cleaning, and brushing can be done at home with basic tools. Save professional grooms for haircuts (if needed) every 6-8 weeks.</li>
        <li><strong>Choose a high-quality diet.</strong> Raw or fresh food may cost more upfront but can reduce vet bills from allergies, dental disease, and obesity-related issues.</li>
      </ul>

      <h2>First-Year vs. Annual Costs</h2>
      <p>Your first year with a !BREED! will be more expensive than subsequent years. Expect to spend an extra !FIRST_YEAR_EXTRA! on:</p>
      <ul>
        <li>Initial vet visit, vaccinations, and microchipping</li>
        <li>Spay/neuter surgery</li>
        <li>Crate, bed, bowls, leash, collar, toys, and other one-time purchases</li>
        <li>Training classes or puppy socialization</li>
      </ul>
      ...
      <details>
        ...
        <div class="faq-a">
          <p>Monthly costs for a !BREED! range from <strong>$!MONTHLY_LOW! to $!MONTHLY_HIGH!</strong>. This includes food, vet care (annualized), insurance, grooming, and supplies. Puppies and seniors typically cost more than healthy adults.</p>
        </div>
      </details>

      <details>
        <summary>Is a !BREED! expensive to own compared to other dogs?</summary>
        <div class="faq-a">
      ...
      <details>
        <summary>What health issues do !BREED_PLURAL! have that affect costs?</summary>
        <div class="faq-a">
          <p>!BREED_PLURAL! are prone to !HEALTH_ISSUES_LC!. These conditions can require ongoing medication, special diets, or surgery — significantly increasing annual veterinary costs beyond the routine care baseline.</p>
        </div>
      </details>
...
<div class="calc-promo">
  ...
  <p>Every dog is different. Use our free calculator to adjust for your !BREED!&rsquo;s age, weight, activity level, and diet.</p>
...
<section class="offers-section">
  ...
  <div class="article-body">
    <div class="highlight-card hc-mint">
      ...
      <p><strong>Frozen raw dog food, made fresh weekly.</strong> Free shipping nationwide. !BREED_PLURAL! on raw diets often show better digestion, healthier coats, and fewer allergy symptoms — which means fewer vet visits.</p>
...
def generate_page(name, data, args=None):
    if args is None:
        args = build_args(name, data)
...
def main():
    ...
    if target_breed:
        if target_breed in DOG_BREEDS:
            breeds_to_generate = [target_breed]
        else:
        ...
        for name in PRIORITY:
            if name in DOG_BREEDS:
                breeds_to_generate.append(name)
    
    print(f'Generating {len(breeds_to_generate)} breed pages...')
    
    for name in breeds_to_generate:
        data = DOG_BREEDS[name]
        args = build_args(name, data)
        generate_page(name, data, args)
    
    print(f'Done! {len(breeds_to_generate)} pages in /breeds/')
